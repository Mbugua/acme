...
class Route(models.Model):
    ...
    def get_search_route(cls, search_departure_location, search_arrival_loaction):
        ...
        found_routes = cls.objects.filter(departure_location=search_departure_location).filter(
            destination_location=search_arrival_loaction)
...
class Bus(models.Model):
    ...
    '''
    bus_organisation = models.ForeignKey(
        BusOrganisation, on_delete=models.CASCADE)
    ...
    def get_bus_organisation_buses(cls, bus_organisation_id):
        ...
        '''
        bus_origanisation_buses = cls.objects.filter(
            bus_organisation=bus_organisation_id)
...
class Schedule(models.Model):
    ...
    def get_travel_estimation(cls, schedule_id):
        ...
        travel_estimation = str(calculate_travel_estimation.seconds//3600) + \
            ' hours ' + str(calculate_travel_estimation.seconds //
                            60 % 60) + ' minutes'
    ...
    def get_departure_buses(cls, departure_date, route_id):
        ...
        departure_datetime = datetime.combine(
            departure_date, time(tzinfo=timezone.get_current_timezone()))
        ...
        found_buses = cls.objects.filter(
            departure_time__range=(departure_datetime, next_date))
